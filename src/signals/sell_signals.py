...
def generate_sell_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ## alt sell condition
    alt_sell = (
                (((df['RSI_hi'].shift(1) < df['RSI'].shift(1)) | (df['RSI_hi'].shift(2) < df['RSI'].shift(2)))
    ...
    new_ema_sell_condition = ((((((df['High'].shift(6) > df['BBU'].shift(6)) & (df['volume_profile'].shift(6) == 1)) & ((df['High'].shift(5) > df['BBU'].shift(5)) & (df['volume_profile'].shift(5) == 0)))
    ...
    uptrend_ema_sell_signal = (
                              # (((((df['EMA_Trend'] == 'Uptrend') & (df['Trend'] == 'Uptrend')) & ((df['regime']== 'downtrend') | (df['regime']== 'sideways')))
                              ...
                              #   & ((((df['Open'] - df['Close'])/ df['Open'])*100 >= 1.10) | (df['regime'] != 'sideways'))
                              ...
                              (((df['EMA_Trend'] == 'Uptrend') & (df['Trend'] == 'Uptrend') & (df['regime']== 'sideways'))
                                ...
                                & (((df['BBM'] - df['Close'])/df['BBM'])*100 > 0.1))
                              # |
                              ...
                              | #keep it
                              ((((df['EMA_Trend'].shift(1) == 'Uptrend') & (df['Trend'].shift(1) == 'Uptrend')) 
                                |((df['EMA_Trend'].shift(2) == 'Uptrend') & (df['Trend'].shift(2) == 'Uptrend')))
                                & (df['EMA_Trend'] == 'Flat') & ((df['EMA_Angle_Degree']) > 190)
                                & ((df['Low'].shift(1).rolling(window=6).mean() > df['Low']) | (df['Low'].shift(1).rolling(window=7).mean() > df['Low']))
                                & ((df['BBU_Angle_Degree']) > (df['BBU_Angle_Degree'].shift(1))) & (df['BBM'] > df['Close'])
                                & ((df['BBU_Angle_Degree'].shift(1).rolling(window=6).mean() < df['BBU_Angle_Degree'])
                                   | (df['BBU_Angle_Degree'].shift(1).rolling(window=7).mean() < df['BBU_Angle_Degree']))
                                & ((df['EMA_Angle_Degree']) > (df['EMA_Angle_Degree'].shift(1))) 
                                & (df['EMA9'] > df['Close']) & (df['BBM'] > df['Close'])
                                & (((df['EMA9'].shift(1) < df['Close'].shift(1)) & (df['BBM'].shift(1) < df['Close'].shift(1)))
                                  | ((df['EMA9'].shift(2) < df['Close'].shift(2)) & (df['BBM'].shift(2) < df['Close'].shift(2))))
                                & (df['BBL'] > df['BBL'].shift(1)) & (df['volume_profile'] == 0))
                              )
    
    uptrend_ema_sell_signal_2 = (
                                ((df['EMA_Trend'] == 'Uptrend') & (df['Trend'] == 'Uptrend') &  (df['volume_profile'] == 0) & (df['volume_profile'].shift(1) == 0)
                                & (((df['Close'].shift(1) >= df['EMA9'].shift(1)) & (df['Close'].shift(1) >= df['BBM'].shift(1))) 
                                   | ((df['Close'].shift(2) >= df['EMA9'].shift(2)) & (df['Close'].shift(2) >= df['BBM'].shift(2))))
                                & (df['Close'] < df['EMA9']) & (df['Close'] < df['BBM']) & (df['Close'].shift(1) >= df['Close'])
                                & (df['EMA_Angle_Degree'] > 185) & (df['BBU_Angle_Degree'].shift(1) > 180) & (df['BBU_Angle_Degree'] > 190) 
                                & ((df['BBU_Angle_Degree'].shift(1) < df['BBU_Angle_Degree']) | (df['BBU_Angle_Degree'] > 220))
                                & ((df['BBL_Angle_Degree'].shift(1) > df['BBL_Angle_Degree']) | (df['BBL_Angle_Degree'].shift(1) < df['BBL_Angle_Degree'].shift(2)))
                                & (df['BBL_Angle_Degree'] < 130) & (((df['EMA9'] - df['Close'])/df['EMA9'])*100 > 0.70)
                               )
                              #  |
                              #  ((df['EMA_Trend'] == 'Uptrend') & (df['Trend'] == 'Uptrend') 
                              #     & (((df['Close'].shift(1) >= df['EMA9'].shift(1)) & (df['Close'].shift(1) >= df['BBM'].shift(1))) 
                              #       | ((df['Close'].shift(2) >= df['EMA9'].shift(2)) & (df['Close'].shift(2) >= df['BBM'].shift(2))))
                              #     & (df['volume_profile'] == 0) & (df['volume_profile'].shift(1) == 0)
                              #     & (df['Close'] < df['EMA9']) & (df['Close'] < df['BBM']) & (df['Close'].shift(1) >= df['Close'])
                              #     & (df['EMA_Angle_Degree'] > 185) & (df['BBU_Angle_Degree'] > 184) 
                              #     & ((df['BBU_Angle_Degree'].shift(1) > 180) | (df['EMA_Angle_Degree'] > 220)) 
                              #     & ((df['BBU_Angle_Degree'].shift(1) < df['BBU_Angle_Degree']) | (df['BBU_Angle_Degree'] > 220)
                              #       | (df['BBM_Angle_Degree'].shift(1) < df['BBM_Angle_Degree']))
                              #     & (((df['EMA9'] - df['Close'])/df['EMA9'])*100 > 0.55)
                              #     & ((df['volume_profile'].shift(2) == 0) | (((df['EMA9'] - df['Close'])/df['EMA9'])*100 > 2))
                              #   )
                               )
    ...
    ema_downside_sell = (((df['regime'] == 'sideways') & (df['volume_profile'] == 0)
                        ...
                        | ((((df['High'].shift(6) > df['BBU'].shift(6)) & (df['volume_profile'].shift(6) == 1) & (df['volume_profile'].shift(5) == 0) & (df['Trend'].shift(5) == 'Uptrend') & (df['regime'].shift(5) == 'other'))
                            | ((df['High'].shift(7) > df['BBU'].shift(7)) & (df['volume_profile'].shift(7) == 1) & (df['volume_profile'].shift(6) == 0) & (df['Trend'].shift(6) == 'Uptrend') & (df['regime'].shift(6) == 'other'))
                            | ((df['High'].shift(5) > df['BBU'].shift(5)) & (df['volume_profile'].shift(5) == 1) & (df['volume_profile'].shift(4) == 0) & (df['Trend'].shift(4) == 'Uptrend') & (df['regime'].shift(4) == 'other')))
                           & (df['volume_profile'] == 0) & (df['regime'] == 'sideways') & (df['volume_profile'].shift(1) == 0)
                           & (df['Close'] < df['EMA9']) & (df['Low'].shift(1) < df['EMA9'].shift(1))
                           & (df['BBU_Angle_Degree'] > 200) & (df['BBU_Angle_Degree'].shift(1) < df['BBU_Angle_Degree'])
                           & (df['BBU_Angle_Degree'].shift(2) < df['BBU_Angle_Degree'].shift(1)) & (df['BBU_Angle_Degree'].shift(3) < df['BBU_Angle_Degree'].shift(2))
                           & (df['EMA_Angle_Degree'] > 185)
                           & (df['BBL_Angle_Degree'] < df['BBL_Angle_Degree'].shift(1))
                           & ((df['EMA9'] - df['BBM']) < (df['EMA9'].shift(1) - df['BBM'].shift(1)))
                           & ((df['EMA9'].shift(2) - df['BBM'].shift(2)) > (df['EMA9'].shift(1) - df['BBM'].shift(1)))
    ...
    downtrend_bbl_sell_signal = ((((df['EMA_Trend'] == 'Downtrend') | (df['EMA_Trend'] == 'Flat')) 
                                ...
                                | (((df['EMA_Trend'] == 'Downtrend') | (df['EMA_Trend'] == 'Flat')) 
                                & (df['Trend'] == 'Downtrend') & (df['volume_profile'] == 0) & (df['volume_profile'].shift(1) == 0)
                                & (df['volume_profile'].shift(2) == 0) & (df['High'].shift(2) > df['BBU'].shift(2))
                                & (df['Low'].shift(1).rolling(window=4).mean() > df['Low'])
                                & (df['Close'] < df['EMA9']) & (df['Close'] < df['BBM'])
                                & (df['Low'].shift(1) < df['EMA9']) & (df['Low'].shift(1) < df['BBM'])
                                )
                                |(((df['EMA_Trend'] == 'Downtrend') | (df['EMA_Trend'] == 'Flat'))
                                ...
                                & (df['volume_profile'].shift(2) == 0) & (df['High'].shift(2) > df['BBU'].shift(2))
                                & (df['Low'].shift(1).rolling(window=4).mean() > df['Low'])
                                & (df['Close'] < df['EMA9']) & (df['Close'] < df['BBM']) & (df['BBM'].shift(1) > df['BBL'])
                                & (df['Low'].shift(1) < df['EMA9']) & (df['Low'].shift(1) < df['BBM'])
                                & (((df['Low'] - df['BBL'])/df['Low'])*100 <= 0.75))
                                )
    test_past_signal = (
                        ...
                        ((((df['Downtrend_Reverse_Buy_Signal'].shift(1) == True) | (df['Downtrend_Reverse_Buy_Signal'].shift(2) == True))
                            ...
                            & (((df['EMA9'].shift(1) - df['EMA9']) / df['EMA9'].shift(1))*100 > 0.10)
                            & (df['Volume'] > df['Volume'].shift(1))
                         )
                        |((df['Downtrend_Reverse_Buy_Signal'].shift(1) == True) & (df['High'].shift(1) < df['BBM'].shift(1))
                            & (df['EMA9'] < df['BBM']) & ((df['EMA9'].shift(1) < df['BBM'].shift(1)) | (df['EMA9'].shift(2) < df['BBM'].shift(2)))
                            & volume_profile_red & (df['BBM_Angle_Degree'] > 240) & (df['EMA_Angle_Degree'] > 220) & (df['BBU_Angle_Degree'] > 250) 
                            & (((((df['Open']) - df['Close']) / df['Open'])*100 > 0.55) 
                                | ((df['BBM'] - df['EMA9']) > (df['BBM'].shift(1) - df['EMA9'].shift(1))))
                            & (((df['EMA9'].shift(1) - df['EMA9']) / df['EMA9'].shift(1))*100 > 0.10)
                            & ((df['Low'] < df['Low'].shift(1)) & (((df['Low'].shift(1) - df['Low']) / df['Low'].shift(1))*100 > 0.05)) 
                         ))
                        ...
                        | ((((df['condition_supreme_low_crossover'].shift(1) == True) 
                            & (df['Close'].shift(1) > df['EMA9'].shift(1)) & (df['Close'].shift(2) > df['EMA9'].shift(2)) 
                               ...
                               & (df['High'] > df['BBU']) & (df['regime'].shift(1) == 'sideways'))
                            | 
                              ...
                              & (df['Close'].shift(1) > df['EMA9'].shift(1)) & (df['Close'].shift(2) > df['EMA9'].shift(2)) 
                                 ...
                                 & ((df['High'] > df['BBU'])
                                  |((df['EMA_Angle_Degree'] > df['EMA_Angle_Degree'].shift(1)) & (df['EMA_Angle_Degree'].shift(1) > df['EMA_Angle_Degree'].shift(2))
                                    & (df['volume_profile'] == 0) & (df['volume_profile'].shift(1) == 0)
                                    & (df['Close'].shift(4) > df['EMA9'].shift(4)) & (df['BBU_Angle_Degree'] >= 190)))
                                 & (df['regime'].shift(2) == 'sideways')))
                        ...
                        | (((df['Opening_buy'].shift(2) == True) | (df['Opening_buy'].shift(3) == True))
                          ...
                          & ~(((df['Open'].shift(3) < df['BBL'].shift(3)) & (df['Open'].shift(2) < df['BBL'].shift(2)) 
                            ...
                            & (df['Date'].dt.time < pd.to_datetime('09:20:00').time())).shift(2)
                              | ((df['Open'].shift(3) < df['BBL'].shift(3)) & (df['Open'].shift(2) < df['BBL'].shift(2)) 
                             & ((((df['BBU'].shift(3) - df['Close'].shift(3)))/df['BBU'].shift(3))*100 > 60)
                            & (df['Close'].shift(3) < df['BBL'].shift(3)) & (df['Close'].shift(2) < df['BBL'].shift(2))
                            & (df['Close'].shift(1) > df['BBL'].shift(1)) & (df['Close'] > df['BBL'])
                            ...
                            & (df['Close'] < df['EMA9']) & (df['Close'] < df['BBM'])
                            & (df['Close'].shift(1) < df['Close']) & (df['Close'].shift(1) > df['Close'].shift(2)) 
                            & (df['Date'].dt.time < pd.to_datetime('09:20:00').time())).shift(1)
                           ))
                        )
    ...
    take_profit_sell = (((df['Trend'] == 'Uptrend') 
                      ...
                      & ((df['BBU_Angle_Degree']) > (df['BBU_Angle_Degree'].shift(1))) 
                      & ((df['EMA_Angle_Degree']) > (df['EMA_Angle_Degree'].shift(1)))
                      & (range_pct >= 5) & (df['volume_profile'] == 0) 
                      & (df['Close'] < df['EMA9']) & (df['Close'] < df['BBM'])
                      & ((df['High'].shift(1) > df['BBU'].shift(1)) | (df['High'].shift(2) > df['BBU'].shift(2))
                         | (df['High'].shift(3) > df['BBU'].shift(3)) | (df['High'].shift(4) > df['BBU'].shift(4))
                         | (df['High'].shift(5) > df['BBU'].shift(5)) | (df['High'].shift(6) > df['BBU'].shift(6)))
                      & ((df['Low'].shift(1).rolling(window=6).mean() > df['Low']) | (df['Low'].shift(1).rolling(window=7).mean() > df['Low'])))
                    |(((df['BBU_Angle_Degree'].shift(1) <= 100) | (df['BBU_Angle_Degree'].shift(2) <= 100) | (df['BBU_Angle_Degree'].shift(3) <= 100)
                       |(df['BBU_Angle_Degree'].shift(4) <= 100) | (df['BBU_Angle_Degree'].shift(5) <= 100) | (df['BBU_Angle_Degree'].shift(6) <= 100))
                      & (df['BBU_Angle_Degree'] >= 100) & (df['Trend'] == 'Uptrend')
                      & ((df['BBU_Angle_Degree']) > (df['BBU_Angle_Degree'].shift(1))) & ((df['EMA_Angle_Degree']) > (df['EMA_Angle_Degree'].shift(1)))
                      & (df['volume_profile'] == 0) & (df['Low'] < df['EMA9'])
                      & (df['Close'].shift(1) > df['EMA9'].shift(1)) & (df['Close'].shift(2) > df['EMA9'].shift(2))
                      & ((df['Low'].shift(1).rolling(window=6).mean() > df['Low']) | (df['Low'].shift(1).rolling(window=7).mean() > df['Low'])))
                    # |((df['Trend'] == 'Uptrend') 
                    #   & ((df['BBU_Angle_Degree']) > (df['BBU_Angle_Degree'].shift(1))) & ((df['BBU_Angle_Degree'].shift(1)) > (df['BBU_Angle_Degree'].shift(2)))
                    #   & ((df['EMA_Angle_Degree']) > (df['EMA_Angle_Degree'].shift(1))) & ((df['EMA_Angle_Degree'].shift(1)) > (df['EMA_Angle_Degree'].shift(2)))
                    #   & ((((df['High'].shift(3) > df['BBU'].shift(3)) & (df['volume_profile'].shift(3) == 1)) & ((df['High'].shift(2) > df['BBU'].shift(2)) & (df['volume_profile'].shift(2) == 0)))
                    #    | (((df['High'].shift(4) > df['BBU'].shift(4)) & (df['volume_profile'].shift(4) == 1)) & ((df['High'].shift(3) > df['BBU'].shift(3)) & (df['volume_profile'].shift(3) == 0)))
                    #    | (((df['High'].shift(5) > df['BBU'].shift(5)) & (df['volume_profile'].shift(5) == 1)) & ((df['High'].shift(4) > df['BBU'].shift(4)) & (df['volume_profile'].shift(4) == 0))))
                    #   & (df['Close'].shift(1) < df['Close'].shift(2)) & (df['Close'] < df['Close'].shift(1)) & (df['volume_profile'] == 0) & (df['volume_profile'].shift(1) == 0)
                    #   & ((df['Low'].shift(1).rolling(window=6).mean() > df['Low']) | (df['Low'].shift(1).rolling(window=5).mean() > df['Low']))
                    #   & (df['Low'] < df['EMA9']) & (df['Low'] < df['BBM']) & ((df['Close'] < df['EMA9']) | (df['Close'] < df['BBM'])))
                    )
    
    df['Sell_Signal'] =  ((condition_close_all_positions)
                        #  | delete (condtion_ema_bbm_sell)
                        #  | (alt_sell) #delete 
                         | (condition_mfi_percentile_low)
                         | (condition_exit_at_top_2)
                        #  | delete (bbu_angle_and_candle_high_sell)
                         | (bbu_curve_sell_signal)
                         | (rsi_pct_bbu_angle_sell) 
                         | (new_ema_sell_condition)
                        #  | (downtrend_ema_sell_signal) #delete 
                         | (ema_downside_sell)
                        #  | (sideways_regime_bbu_top_sell) #delete
                        # |  (bottleneck_sell_condition) #delete
